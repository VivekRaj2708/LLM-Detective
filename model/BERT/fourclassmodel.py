...
"""
Script to train a text detector that predicts the model (llama, mistral, aya, falcon) 
based on text input and a categorical 'type' feature.
"""
...
# Model parameters
# We're now predicting the model (llama, mistral, aya, falcon) -> 4 classes
NUM_MODEL_CLASSES = 4  # llama, mistral, aya, falcon
...
def evaluate(model, data_loader, model_criterion, device):
    ...
    with torch.no_grad():
        # Prepare accumulators for multi-class metrics
        num_classes = NUM_MODEL_CLASSES
        tp_per_class = [0 for _ in range(num_classes)]
        fp_per_class = [0 for _ in range(num_classes)]
        fn_per_class = [0 for _ in range(num_classes)]
        ...
        for batch in tqdm(data_loader, desc="Evaluating", leave=False):
            ...
            # Accumulate per-class TP/FP/FN for multi-class metrics
            for c in range(num_classes):
                tp_per_class[c] += ((model_preds == c) & (model_labels == c)).sum().item()
                fp_per_class[c] += ((model_preds == c) & (model_labels != c)).sum().item()
                fn_per_class[c] += ((model_preds != c) & (model_labels == c)).sum().item()


    avg_model_loss = total_model_loss / len(data_loader)
    model_accuracy = correct_model_preds / total_samples if total_samples > 0 else 0.0

    # Compute per-class precision/recall/f1 and then macro-average
    precisions = []
    recalls = []
    f1s = []
    for c in range(num_classes):
    ...
    return avg_model_loss, model_accuracy, macro_precision, macro_recall, macro_f1
...
def main():
    Best_acc = 0.8
    # Removed argparse setup for notebook execution
    ...
    # args = parser.parse_args() # Removed parsing

    # Use predefined constants instead of args
    ...
    max_length = MAX_LENGTH
    lr = LEARNING_RATE
    #resume = "best_tinybert.pth" # Set resume explicitly if needed, or keep as None for no resume
    resume = None
    save_path = "best_.pth" # Path to save final checkpoint 
    ...
    if resume:
        try:
            ...
            print(f"Checkpoint file not found: {resume}", file=sys.stderr)
            raise
        except Exception as e:
    ...
    for epoch in range(start_epoch, epochs):
        ...
        val_model_loss, val_model_acc, val_precision, val_recall, val_f1 = evaluate(
        ...
        print(f"  Val Model Precision: {val_precision:.4f}")
        print(f"  Val Model Recall: {val_recall:.4f}")
        print(f"  Val Model F1 Score: {val_f1:.4f}")
